...
export class Game {
  ...
  constructor(player_a: GameSocket, player_b: GameSocket) {
    ...
    // setup events
    this.player_o.once("disconnect", () => {
      this.status = "x_won";
    });
    this.player_x.once("disconnect", () => {
      this.status = "o_won";
    });
    this.player_o.once("game_ready", callback => {
      callback(this.game_state);
    });
    this.player_x.once("game_ready", callback => {
      callback(this.game_state);
    });
    this.player_x.once("game_leave", () => {
  ...
  setup_move_event() {
    ...
    this.current_turn_player.once("game_move", (cellIndex, callback) => {
      if (this.status === "in-progress") {
